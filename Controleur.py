...
### TP 05 ##################################################################### 
    ...
    def methodSceneZBuffer(self, indcptobj, d, larg, haut, listesommetsdansreperecamera):
        ...
        for tr in self.scene.listeobjets[indcptobj].listeindicestriangle :
            ...
            #On rentre ici les normales de chaque points
            for ii in range(3):
                fac.normale.append(self.scene.listeobjets[indcptobj].listenormales[self.scene.listeobjets[indcptobj].listeindicesnormales[i][ii] - 1])
            
            #TP06####################################################"
            fac.coefs=self.scene.listeobjets[indcptobj].listecoefs[i]
