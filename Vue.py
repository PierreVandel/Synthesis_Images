...
class VueCourbes(object):
    ...
    def callbackNouvellesceneZBuffer(self):
        
        self.controleur = Controleur.ControleurCourbes()
        self.majAffichage()
        
        self.outilsCourant = self.controleur.initzbuffer()
        #passage des parametres de l image pour l allocation du zbuffer
        if self.outilsCourant :
            self.outilsCourant(self.largeur, self.hauteur)
    ...
    def executer(self):
        ...
        filemenu.add_command(label="Import scene Z-Buffer", command=self.callbackNouvellesceneZBuffer)
